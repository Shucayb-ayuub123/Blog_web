...
const Menu : React.FC<categoty> = ({cat}) => {
    ...
    useEffect(() => {
     const fetchData = async () =>{
      try {
        const response  = await fetch(`http://localhost:3000/api/post/getPost/?cat=${cat}`)
  ...
  return (
    <div className="flex justify-start items-center flex-col space-y-5">
      ...
      {post?.map((post) => (
        <div className="flex justify-start  flex-col"  key={post.id}>
